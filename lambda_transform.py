...
def lambda_handler(event, context):
    ...
    for data in spotify_data:
        ...
        # to dataFrame and data transformations
        ...
        # df to string to csv to s3
        song_fileName = 'transformed_data/songs_data/song_transformed_' + str(datetime.now()) + '.json'
        song_buffer = StringIO()
        song_df.to_csv(song_buffer, index=False)
        song_content = song_buffer.getvalue()
        s3.put_object(Bucket=Bucket, Key=song_fileName, Body=song_content)
        
        album_fileName = 'transformed_data/album_data/album_transformed_' + str(datetime.now()) + '.json'
        album_buffer = StringIO()
        album_df.to_csv(album_buffer, index=False)
        album_content = album_buffer.getvalue()
        s3.put_object(Bucket=Bucket, Key=album_fileName, Body=album_content)
        
        artist_fileName = 'transformed_data/artist_data/artist_transformed_' + str(datetime.now()) + '.json'
